...
import visited from './app/config/Global';
import WebClient from './app/config/WebClient';
...
import MessageToArtistViewController from "./app/routes/MessageToArtistViewController"
...
import VariantsController from "./app/routes/VariantsController"    // added by Wang Feb 7th, 2019
import AddMaterialController from "./app/routes/AddMaterialController"    // added by Wang Feb 7th, 2019
import AddSizeController from "./app/routes/AddSizeController"    // added by Wang Feb 7th, 2019
import TermsViewController from "./app/routes/TermsViewController"
import PolicyViewController from "./app/routes/PolicyViewController"
import MobileVerificationController from "./app/routes/MobileVerificationController"
...
function registerAllScreens() {
  ...
  registerScreen('Artwork.MessageToArtistViewController', MessageToArtistViewController, {});
  ...
  registerScreen('Artwork.VariantsController', VariantsController, {});
  registerScreen('Artwork.AddMaterialController', AddMaterialController, {});
  registerScreen('Artwork.AddSizeController', AddSizeController, {});
  registerScreen('Artwork.TermsViewController', TermsViewController, {});
  registerScreen('Artwork.PolicyViewController', PolicyViewController, {});
  registerScreen('Artwork.MobileVerificationController', MobileVerificationController, {});
}
...
/*
  WebClient.postRequest(Settings.URL.GET_VISITED_LIST, {
    'unique_device_id': Utility.deviceId + '',
  }, (response, error) => {
    
    if (error == null) {
        if (response) {
          visited.cartComponent = (response[0].cartView == 'true');
          visited.inventoryComponentListed = (response[0].inventoryView == 'true');
          visited.commissionsComponentOngoing = (response[0].commisionView  == 'true');
        }
    } else {
...
*/
  ...
  User.getAsyncData((cartView) => {
    if (cartView != undefined) {
      if (cartView == 'true') {
        visited.cartComponent = true;
      }
    }
  }, 'cartView');
  
  User.getAsyncData((inventoryView) => {
    if (inventoryView != undefined) {
      if (inventoryView == 'true') {
        visited.inventoryComponentListed = true;
      }
    }
  }, 'inventoryView');

  User.getAsyncData((commisionView) => {
    if (commisionView != undefined) {
      if (commisionView == 'true') {
        visited.commissionsComponentOngoing = true;
      }
    }
  }, 'commisionView');
...
function setUpNavigation(screenName) {
  ...
  // if (__DEV__){
  //   screenName = 'Artwork.SignupController';
  // }
