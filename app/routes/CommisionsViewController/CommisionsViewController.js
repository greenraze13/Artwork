...
class CommisionsViewController extends Component {
    ...
    componentWillUnmount() {
        if (this.state.isOngoingVisited) {
            if (this.state.arrOngoingCommissions.length > 0) {
    ...
    btnOngoingJobTapped() {
        this.setState({ isDataReceived: false, currentState: ONGOING, isOngoingVisited: true });
    ...
    getOnGoingJobList() {
        ...
        if (this.state.arrOngoingCommissions.length > 0) {
            Utility.updateSwipeViewStatus('commisionView');
        }
    }
    ...
    renderOngoingJobViewCell(rowData, secId, rowId, rowMap) {
        ...
        var index = rowId;
        /********** added by Wang - start ***********/
        var disableRightSwipeFlag = jobItem.is_user_job_delete_to_cart == 1 ? false : (jobItem.order_status_id == Utility.OrderStatus.NONE) ? true : false;
        var disableLeftSwipeFlag = (jobItem.order_status_id == Utility.OrderStatus.NONE) ? true : false;
        /********** added by Wang - end   ***********/
        return (
            <SwipeRow
                ...
                leftOpenValue={75}
                rightOpenValue={-75}
                preview={!index && !visited.commissionsComponentOngoing? true : false}         // added by Wang
