...
class ArtistPortalViewController extends Component {
    ...
    saveArtistTapped() {
        ...
        if (Utility.user.is_artist_approved == 3) {
            ...
            if (this.state.artist_access_code.toString().trim() == '') {
                ...
                // else if ((this.state.latitude == 0 || this.state.longitude == 0)) {      // commented by Wang 2019/03/24
                //     Utility.showToast(Utility.MESSAGES.please_enter_your_address)
                // }
                else if (this.state.address.toString().trim() == "") {
    ...
    saveArtist() {
        ...
        WebClient.postRequest(Settings.URL.REQUEST_FOR_ARTIST, {
            ...
            if (error == null) {
                if (response.user_id) {
                    if (response.is_artist_approved == 1) {
                        // Utility.showToast(Utility.MESSAGES.now_you_are_artist);

                        setTimeout(() => {
                            Utility.resetTo('ArtistProfileViewController', {
                                isFromArtistPortal: true
                            })
                        }, 500);
                    } else if (response.is_artist_approved == 3) {
                        ...
                        
                        {
                            ...
                            <View style={{ marginTop: 25, flex: 1, alignItems: 'center', paddingVertical: 10 }}>
                                <INTButton buttonStyle={{ paddingHorizontal: 20, backgroundColor: Colors.blueType1, justifyContent: 'center' }}
                                    ...
                                    onPress={() => this.saveApprovalCodeTapped()}
                                />
                            </View>
                        }
                                ...
                                Utility.user.is_artist_approved == 3 ?
                                    ...
                                    <View >
                                        <View style={styles.subTitleView}>
                                            ...
                                            <Text style={styles.txtIntrested}>3-months Free Trial!</Text>
                                            <Text style={styles.txtFillOut}>Fill out the application below and we will get back to you shortly!</Text>
                                        </View>
