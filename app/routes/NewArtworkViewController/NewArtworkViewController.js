...
// console.log("this.props.artwork_id ============== " + this.props.artwork_id);
        this.state = {
...
// console.log("Utility.user.latitude ============ " + Utility.user.latitude);
// console.log("Utility.user.longitude ============= " + Utility.user.longitude);
// console.log("isEditArtwork ============= " + this.state.isEditArtwork);
        // if (Utility.user != undefined && (Utility.user.latitude == 0 || Utility.user.longitude == 0)) {      commented by Wang 03/29/2019
        // } else 
        {
    ...
    onNavigatorEvent(event) {
        switch (event.id) {
            case 'willAppear':
                // this.updateAddressAlert();           // commented by Wang 03/27/2019
                break;
            case 'didAppear':
                break;
            case 'willDisappear':
        ...
        WebClient.postRequest(Settings.URL.GET_ARTWORK_DETAIL_V2, params, (response, error) => {
            ...
            if (error == null) {
                if (response) {
                    ...
                    response.artwork_variants.forEach(element => {
                        var tmp = element.name.split("/");
                        var bExistSize = false;
                        var bExistMaterial = false;

                        size.forEach(element => {
                            if(element.name == tmp[0]) {
                                bExistSize = true;
                            }
                        });
                        if(!bExistSize) {
                            size.push({name: tmp[0]});
                        }

                        material.forEach(element => {
                            if(element.name == tmp[1]){
                                bExistMaterial = true;
                            }
                        });
                        if(!bExistMaterial) {
                            material.push({name: tmp[1]});
                        }
                    });
                    
                    tmpVariants.size = size;
                    tmpVariants.material = material;

                    this.setState({variants: tmpVariants});
                    ...
                    this.setState({
                        ...
                        pickup_address: response.pickup_address,        // added by wang
                        variantsCombine: response.artwork_variants
                    })
                } else {
                    this.leftBtnTaaped();
                }
    ...
    btnLongPress(number) {
        ...
        Alert.alert(
            null,
            'Do you want to detele this image?',
            [
                {
                    text: 'Cancel',
                    onPress: () => {},
                ...
                {text: 'OK', onPress: () => {
                    ...
                    photoUpdate.photoIDUrl = null;
                    photoUpdate.photoServerURL = "";
                    photoUpdate.photoServerImageName = "";
...
// console.log("=====================" + JSON.stringify(rowData));
        return (
            <View>
                {((rowData.item.photoIDUrl != undefined && rowData.item.photoIDUrl != null) || rowData.item.photo != '' || rowData.item.photoServerURL != '')
                    ?
                    // <TouchableOpacity onLongPress={() =>this.btnLongPress(rowData.item.key)} onPress={() => this.btnSelectImageTapped(rowData.item.key)}>
    ...
    onMainUniqueRepeatableTapped(status) {
        this.setState({ isRepeatable: status });
    }

    onUniqueRepeatableTapped(status, idx) {
        this.setState({ variantsCombine: 
            a = this.state.variantsCombine.map(function(data) { return data.name != item.name ? data : {name: data.name, price: value}; })
        })
    }
    ...
    selectImage(number) {
        if (this.state.hasCameraPermission && this.state.hasWritePermission) {
            ...
            if(photoUpdate.photoIDUrl == null && photoUpdate.photoServerURL == "" && photoUpdate.photoServerImageName == "") {
                ...
                ImagePicker.showImagePicker({
                    title: 'Select Photo',//customButtons: [ { name: 'fb', title: 'Choose Photo from Facebook' } ],
                    customButtons: [
                        { name: 'customOptionKey', title: 'Delete Artwork' },
                    ],
                    storageOptions: {
                        skipBackup: true, path: 'images'
                    }
    ...
    addArtworkTapped() {
        ...
        for (var i = 0; i < this.state.arrArtPhotos.length; i++) {
            ...
            if (element.photoIDUrl != null || element.photo.length > 0 || element.photoServerURL.length > 0) {
                imageCount = imageCount + 1;
            }
        }

        var emptyFlag = true;
        for(var i=0; i<this.state.variantsCombine.length;i++) {
            if(this.state.variantsCombine[i].price.trim()){
                emptyFlag = false;
                break;
            }
        }

/*        if (Utility.user != undefined && (Utility.user.latitude == 0 || Utility.user.longitude == 0)) {   commented by Wang 03/27/2019
            this.updateAddressAlert();
        ...
        } else if(emptyFlag && this.state.variantsCombine.length) {
            Utility.showToast(Utility.MESSAGES.please_enter_variants_price);
...
// console.log("NewArtworkViewController::uploadImage ==========1111111111");
            ...
            if (number == (this.state.arrArtPhotos.length - 1)) {
// console.log("NewArtworkViewController::uploadImage ==========222222");
                this.saveArtwork();
            } else {
                let photoUpdate = this.state.arrArtPhotos[number];
    ...
    saveArtwork() {
        ...
        var variants = this.state.variantsCombine.filter(data => data.price > 0);

        variants.forEach(element => {
            if(parseInt(element.price.trim()) > 0) {
                var tmp = parseInt(element.quantity);
                total_quantity = total_quantity + tmp;
            }

            // if(element.price > 0 && price > element.price) {
            //     price = element.price;
            //     size = element.name;
            // }
        });

        total_quantity += this.state.repeatableQuantity;
        ...
        var paramRequest = {
            ...
            'pickup_address': this.state.pickup_address + '',             // added by Wang
            'variants': variants,
            total_quantity,
        }

        // console.log('variantsCombine ' + JSON.stringify(this.state.variantsCombine));
        // console.log('paramRequest ' + JSON.stringify(paramRequest));

        WebClient.postRequest(Settings.URL.ADD_ARTWORK_V2, paramRequest, (response, error) => {
            this.setState({ spinnerVisible: false });
            if (error == null) {
    ...
    /* added by Wang Feb/8/2019 - start */
    onAddVariantsCallBack(data) {
        // console.log("data ============= ", JSON.stringify(data));
        // format of data : {size: [{name:'aaa'},{name:'bbb'}]}
        this.setState({variants: data});
        var tmp = [];
        var sign = "/";

        if(data.size.length) {
            data.size.forEach(sData => {
                if(data.material.length) {
                    data.material.forEach(mData => {
                        tmp.push({name: sData.name + sign + mData.name, price: '', isRepeatable: 0, quantity: 1});
                    });
                } else {
                    tmp.push({name: sData.name, price: '', isRepeatable: 0, quantity: 1});
                }
            });
        } else {
            data.material.forEach(sData => {
                if(data.size.length) {
                    data.size.forEach(mData => {
                        tmp.push({name: sData.name + sign + mData.name, price: '', isRepeatable: 0, quantity: 1});
                    });
                } else {
                    tmp.push({name: sData.name, price: '', isRepeatable: 0, quantity: 1});
...
// console.log("=======================", tmp);
        ...
        this.setState({variantsCombine: tmp});
    }

    // onAddMaterialCallBack() {
    //     this.setState({variants: data});
    // }

    onAddVariantsTapped() {
        // console.log("*************************", JSON.stringify(this.state.variants));
        
        Utility.push("VariantsController", {
            variants: this.state.variants,
            onAddVariantsCallBack: this.onAddVariantsCallBack.bind(this),
            // onAddMaterialCallBack: this.onAddMaterialCallBack.bind(this),
        });
    }

    /* added by Wang Feb/8/2019 - end */
                    ...
                    <KeyboardAwareScrollView style={{ marginTop: 20 }}
                        ...
                        <View style={styles.applicationFormContainer}>
                            ...
                            <View style={styles.applicationFormInnerContainer}>
                                ...
                                {/* {
                                    this.state.variantsCombine.length == 0 ? */}
                            ...
                            <View style={{ flexDirection: 'row', marginTop: 15 }} >
                                <INTButton buttonStyle={[styles.btnUniqueRepeatable, { flex: 0.8 }]}
                                    ...
                                    onPress={() => this.onMainUniqueRepeatableTapped(Utility.ArtworkType.UNIQUE)} />
                                <INTButton buttonStyle={[styles.btnUniqueRepeatable]}
                                    ...
                                    spaceBetweenIconAndTitle={0}
                                    onPress={() => this.onMainUniqueRepeatableTapped(Utility.ArtworkType.REPEATABLE)} />
                            ...
                            <View style={{ marginTop: 5, flex: 1, alignItems: 'center', paddingVertical: 10 }}>
                                <INTButton buttonStyle={{ paddingHorizontal: 20, backgroundColor: Colors.blueType1, justifyContent: 'center' }}
                                    title={this.state.isEditArtwork ? "Change Options" : 'Add Options'}
                                    titleStyle={{ color: 'white' }}
                                    onPress={() => this.onAddVariantsTapped()}
                                />
                            </View>
    {
        this.state.variantsCombine.map((item, idx) =>
            <View style={[{flex: 1, marginTop: 10, marginLeft: 5, borderColor: Colors.blueType1, borderWidth: 1, borderRadius: 5, marginBottom: 5, paddingBottom: 10}]}>
                <View style={{flex: 1, flexDirection: 'row', paddingLeft: 10, marginBottom: 10}}>
                    <View style={{flex: 3, paddingTop: 10}}>
                        <Text style={{color: Colors.blueType1}}>{item.name}</Text>
                    </View>
                    <View style={{flexDirection: 'row', flex: 1, alignItems: 'flex-end'}}>
                        <View style={{flex:2, alignItems: 'flex-end'}}>
                            <TextField
                                ...
                                onChangeText={(value) => {
                                        this.setState({ variantsCombine: 
                                            a = this.state.variantsCombine.map(function(data) { return data.name != item.name ? data : {name: data.name, price: value, isRepeatable: data.isRepeatable, quantity: data.quantity}; })
                                        })
                                    }
                                }
                                value={this.state.variantsCombine[idx].price + ''}
                            />
                        </View>
                        <View style={{paddingTop:10, flex:1, alignItems: 'flex-start'}}>
                            <Text style={{color:Colors.blueType1}}>$</Text>
                        </View>
                    </View>
                </View>
                <View style={{flex: 1, flexDirection: 'row'}}>
                    <View style={{flex: 2, flexDirection: 'row'}}>
                        <View style={{flex: 1, alignItems: 'flex-start'}}>
                            <INTButton buttonStyle={[styles.btnUniqueRepeatable]}
                                title="Unique"
                                titleStyle={styles.btnTitleOptions}
                                icon={(this.state.variantsCombine[idx].isRepeatable == 0) ? Images.radioSelected : Images.radioNotSelected}
                                spaceBetweenIconAndTitle={0}
                                onPress={() => {
                                        this.setState({ variantsCombine: 
                                            a = this.state.variantsCombine.map(function(data) { return data.name != item.name ? data : {name: data.name, price: data.price, isRepeatable: Utility.ArtworkType.UNIQUE, quantity: data.quantity}; })
                                        })
                                    }
                                }
                            />
                        ...
                        <View style={{flex: 1, alignItems: 'flex-start'}}>
                            <INTButton buttonStyle={[styles.btnUniqueRepeatable]}
                                title="Repeatable"
                                titleStyle={styles.btnTitleOptions}
                                icon={(this.state.variantsCombine[idx].isRepeatable == 1) ? Images.radioSelected : Images.radioNotSelected}
                                spaceBetweenIconAndTitle={0}
                                onPress={() => {
                                        this.setState({ variantsCombine: 
                                            a = this.state.variantsCombine.map(function(data) { return data.name != item.name ? data : {name: data.name, price: data.price, isRepeatable: Utility.ArtworkType.REPEATABLE, quantity: data.quantity}; })
                                        })
                                    }
                                }
                            />
                    ...
                    {this.state.variantsCombine[idx].isRepeatable == 1 ?
                        <View style={{flex:1, flexDirection:'row', paddingRight: 5,}}>
                            <View style={{alignItems: 'flex-end', flex: 3}}>
                                <Text style={[styles.titleStyle, {paddingTop: 10}]}>Qty : </Text>
                            </View>
                            <View style={{alignItems: 'flex-start', flex: 2}}>
                                <TextField
                                    ...
                                    onChangeText={(value) => {
                                            this.setState({ variantsCombine: 
                                                a = this.state.variantsCombine.map(function(data) { return data.name != item.name ? data : {name: data.name, price: data.price, isRepeatable: data.isRepeatable, quantity: value}; })
                                            })
                                        }
                                    }
                                    value={this.state.variantsCombine[idx].quantity + ''}
                                />
                            </View>
                        </View> : null
                    }
                            ...
                            // {(this.state.dipatchOptionShipping == 0 && this.state.dipatchOptionDelivery == 0 && this.state.dipatchOptionPickup == 0) ?
                                null : <View style={[styles.applicationFormInnerContainer, { justifyContent: 'flex-start' }]}>
                                    {this.state.dipatchOptionShipping == 1 ?
                                        ...
                                        <View style={{marginRight:10, width: '48%' }}>
                            ...
                            {/* added by Wang - start */}
                            {this.state.dipatchOptionPickup == 1 ?
                                // <View style={{ minWidth: 100 }}>      commented by Wang
                                <View style={{ width: '96%', marginLeft:5, marginTop:10 }}>
                                    <Text style={styles.titleStyle}>Pickup Address</Text>
                                    <TextField
                                        inputStyle={styles.inputText}
                                        autoCorrect={false}
                                        placeholder={""}
                                        ...
                                        returnKeyType="next"
                                        onChangeText={(pickup_address) => this.setState({ pickup_address })}
                                        value={this.state.pickup_address.toString()}
                                    />
                                </View> : null
                            }
                            {/* added by Wang - end */}
