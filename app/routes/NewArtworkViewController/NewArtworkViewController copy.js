...
class NewArtworkViewController extends Component {
    ...
    addArtworkTapped() {
        ...
        } else if (this.state.size.trim() == "" && this.state.variantsCombine.length <= 0) {
        ...
        } else if (this.state.price == "" && this.state.variantsCombine.length <= 0) {
...
// console.log(JSON.stringify(options));
    ...
    saveArtwork() {
        ...
        var price = 99999999;
        var size = '';
        this.state.variantsCombine.forEach(element => {
        ...
        var paramRequest = {
            ...
            'price': this.state.variantsCombine.length ? price + '' : this.state.price + '',
            'preferred_medium_id': this.state.preferred_medium_id + '',
            'size': this.state.variantsCombine.length ? size + '' : this.state.size + '',
            ...
            'variants': this.state.variantsCombine,
        }
            ...
            data.size.forEach(sData => {
                if(data.material.length) {
                    data.material.forEach(mData => {
                        tmp.push({name: sData.name + sign + mData.name, price: ''});
                    });
                } else {
                    tmp.push({name: sData.name, price: ''});
                }
            });
        } else {
                        ...
                        <View style={styles.applicationFormContainer}>
                            ...
                            <View style={{ marginTop: 25, flex: 1, alignItems: 'center', paddingVertical: 10 }}>
                                <INTButton buttonStyle={{ paddingHorizontal: 20, backgroundColor: Colors.blueType1, justifyContent: 'center' }}
                                    title={this.state.isEditArtwork ? "Change Variants" : 'Add Variants'}
...
{/* <View style={{ marginTop: 25, flex: 1, alignItems: 'center', paddingVertical: 10 }}>
    <INTButton buttonStyle={{ paddingHorizontal: 20, backgroundColor: Colors.blueType1, justifyContent: 'center' }}
        title={'Get Variants'}
        titleStyle={{ color: 'white' }}
        onPress={() => this.onGetVariantsTapped()}
    />
</View> */}
