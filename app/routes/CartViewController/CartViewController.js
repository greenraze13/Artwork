...
class CartViewController extends Component {
    ...
    componentDidMount() {
        ...
        if(this.props.fromScreen == "OrderDetailViewController") {
            this.setState({ isDataReceived: false, currentState: PAST_ORDERS });
            this.getPastOrders();
        } else {
            /************************added by Wang - end *******************/
            this.getCartList();
            //console.log("*******************************componentDidMount function" , visited.cartComponent);
        }
    }
    ...
    btnPastOrderClick() {
        if (this.state.arrCartList.length > 0) {
    ...
    getCartList() {
        ...
        WebClient.postRequest(Settings.URL.GET_CART_V2, {
            ...
            if (error == null) {
                if (response) {
                    ...
                    if (this.state.arrCartList.length > 0) {
                        Utility.updateSwipeViewStatus('cartView');
                    }
    ...
    setTotalPrice(arrCartListTemp) {
        ...
        servicefee = (totalDeliveryCost + totalShippingCost + totalPrice) / (1 - Utility.getAWSData.stripe_transaction_fee / 100) - (totalDeliveryCost + totalShippingCost + totalPrice);
        this.setState({
            artworkTotalPrice: totalPrice,
            shippingTotalPrice: totalShippingCost,
            deliveryTotalPrice: totalDeliveryCost,
            servicefee,
            grandTotalPrice: (totalDeliveryCost + totalShippingCost + totalPrice + servicefee)
        });
    }
    ...
    updateItemQuantity(index, cartList, isMinus) {
        ...
        WebClient.postRequest(Settings.URL.ADD_TO_CART_V2, {
            ...
            'unique_device_id': Utility.deviceId + '',
            'selected_variants_id': cartList[index].variants_id,
        }, (response, error) => {
    ...
    renderCartViewCell(rowData, secId, rowId, rowMap) {
        ...
        return (
            <SwipeRow
                ...
                rightOpenValue={-75}
                preview={(position == 0) && !visited.cartComponent ? true : false}       // added by Wang
                previewOpenValue={-75}                                                  // added by Wang
            >
                    ...
                    <View style={styles.itemStyle}>
                        ...
                        <View style={styles.viewArtNameStyle}>
                            <TouchableOpacity onPress={() => this.onArtworkTapped(cart)} activeOpacity={1}>
                                ...
                                <Text style={styles.textVariantsName} numberOfLines={1}>
                                    {cart.variants_title}
                                </Text>
    ...
    getPastOrders() {
        this.setState({ spinnerVisible: true });
        WebClient.postRequest(Settings.URL.GET_ORDER_LIST_V2, {
...
// console.log("goorderDetailScreen() = ", order);
        Utility.push('OrderDetailViewController', { orders: { ...order } })
    }
    /////////////////////////////////////////   ORDER LIST  END ///////////////////////////////////////////////

    componentWillUnmount() {
//        console.log("******************** componentWillUnmount", visited.cartComponent);
        if (this.state.arrCartList.length > 0) {
            visited.cartComponent = true;
        }
    }
    ...
    render() {
        //console.log("*******************************render function", visited.cartComponent);
                            ...
                            this.checkCurrentState(CART) ?
                                this.state.arrCartList.length > 0 ? // check if records available otherwise show no records label
                                    [<View style={{marginBottom:10, marginTop:10, alignItems: "center"}}>
                                        <Text style={{fontWeight: "normal"}} >7-days Free Returns</Text>
                                    </View>,
                                    <SwipeListView
                    ...
                    {this.checkCurrentState(CART) ?
                        this.state.arrCartList.length > 0 ?
                            <View style={{ alignItems: 'flex-end', paddingVertical: 10, marginHorizontal: 10 }}>
                                ...
                                {
                                    (this.state.shippingTotalPrice > 0) ?
                                ...
                                {
                                    (this.state.deliveryTotalPrice > 0) ? 
                                        <View style={{ flexDirection: 'row', marginTop: 4, }}>
                                ...
                                <View style={{ flexDirection: 'row', marginTop: 4, }}>
                                    ...
                                    <Text style={[styles.textTotalPrice, { color: Colors.blueType1 }]}>{Utility.DOLLOR}{Utility.parseFloat(this.state.servicefee)}</Text>
                                </View>
